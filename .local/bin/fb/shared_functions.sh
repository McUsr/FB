...
# We check if it is isn't empty, and if it is, then we check if there is something
# that can pass for a glob in there.
# PARAMETERS:
...
excludeFileHasContents() {
  ...
  # we have always vetted the parameters backup_scheme and symlink_name up front by our Callers.
  backup_scheme="$1"
  symlink_name="$2"
  ...
  if [[ -s "$probe" ]] ; then
    if [[ $DEBUG -eq 0 || $VERBOSE == true || $DRYRUN == true ]] ; then
        echo "$PNAME/${FUNCNAME[0]} : maybe we have an exclude file with contents"
    fi
    grep '[-/.@+a-zA-Z0-9]\+'  < "$probe" &>/dev/null
    return $?
  else
    if [[ $DEBUG -eq 0 || $VERBOSE == true || $DRYRUN == true ]] ; then
      echo "$PNAME/${FUNCNAME[0]} : we DON'T have an exclude file with contents"
    fi
    return 1
  fi
}
...
# GLOBALS : EXCLUDE_FILE : the full path to any exclude file is delivered through this.
...
hasExcludeFile() {
# Maybe we can use globals symlink_name and such as BACKUP_SCHEME for optimization.

  if [[ $# -ne 2 ]] ; then
    echo -e "${0##*/}/${FUNCNAME[0]} I need two parameters BACKUP_SCHEME and SYMLINK_NAME!\nTerminating... " 1>&2
    exit 5
  fi
  ...
  if [ -d "$XDG_BIN_HOME/fb/$backup_scheme/$symlink_name.d" ] ; then
    ...
    if [[ $VERBOSE = true || $DEBUG -eq 0 || $DRYRUN = true ]] ;  then
      echo -e "$PNAME : We don't have  a dropin directory: $XDG_BIN_HOME/fb/$backup_scheme/$symlink_name.d"
      echo -e "$PNAME : We don't have an \"exclude.file\" file: $XDG_BIN_HOME/fb/$backup_scheme/$symlink_name.d/exclude.file"
    fi
    return 1
  fi
}
...
createExcludeFile() {
  if [[ $# -ne 2 ]] ; then echo -e "${0##*/}/${FUNCNAME[0]} : Need two arguments\nTerminates" >&2 ; exit 5 ; fi
  ...
  excludeFileHasContents $SCHEME "$FULL_SYMLINK_NAME"
  if [[  $? -ne 0 ]] ; then
